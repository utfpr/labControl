...
export default function DashboardPage() {
  ...
  const fetchAgenda = async () => {
    ...
    try {
      ...
      const eqReservasRaw = (await eqAgendaRes.json()) as any[];
      const locReservasRaw = (await locAgendaRes.json()) as any[];

      const eqReservas = eqReservasRaw.map(r => ({ ...r, tipo: 'equipamento' }));
      const locReservas = locReservasRaw.map(r => ({ ...r, tipo: 'local' }));

      // Merge and sort by time
      const allReservas = [...eqReservas, ...locReservas].sort((a, b) => {
        const timeA = a.dataHoraInicio || "00:00";
        const timeB = b.dataHoraInicio || "00:00";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-4 border-b pb-2">
              Reservas de Locais
            </h2>
            <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">
              <span>
                Reservas Confirmadas: <strong>{resLocStats.confirmadas}</strong>
                <br />
                Reservas Pendentes: <strong>{resLocStats.pendentes}</strong>
              </span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <div className="flex justify-between text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800 p-3 rounded-lg">
              <span>
                Confirmadas: <strong>{resEqStats.confirmadas}</strong>
                <br />
                Pendentes: <strong>{resEqStats.pendentes}</strong>
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {agendaLoading ? (
              ...
              agendaItems.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors border border-transparent hover:border-slate-100 dark:hover:border-slate-700"
                >
                  <div
                    className={`w-3 h-3 rounded-full mt-1.5 flex-shrink-0 shadow-sm ${
                      item.tipo === "local" ? "bg-amber-500" : "bg-teal-500"
                    }`}
                  ></div>
                  <div>
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400">
                      Horário:{" "}
                      {(() => {
                        const start = item.dataHoraInicio || "";
                        if (!start || start.trim() === "") return "N/A";
                      ...
                      até{" "}
                      {(() => {
                        const end = item.dataHoraFim || "";
                        if (!end || end.trim() === "") return "N/A";
