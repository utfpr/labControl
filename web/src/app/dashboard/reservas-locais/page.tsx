...
import { CalendarRange, Plus, AlertCircle, CheckCircle, XCircle, Clock, CheckSquare } from "lucide-react";
import { useRouter } from "next/navigation";
import ModalNovaReservaLocal from "./ModalNovaReservaLocal";

export default function ReservasLocaisPage() {
  ...
  const buscarReservas = async () => {
    try {
      ...
      const endpoint = (role === 'admin' || role === 'supervisor') 
        ? "http://localhost:3000/reservas-locais" 
        : "http://localhost:3000/reservas-locais/minhas";
  ...
  const alterarStatus = async (id: string, acao: 'aprovar' | 'rejeitar' | 'cancelar' | 'finalizar') => {
    try {
      const token = localStorage.getItem("labcontrol_token");
      const response = await fetch(`http://localhost:3000/reservas-locais/${id}/${acao}`, {
        method: 'PATCH',
        headers: { "Authorization": `Bearer ${token}` }
      });
